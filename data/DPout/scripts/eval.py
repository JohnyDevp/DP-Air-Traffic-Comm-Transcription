...
def compute(test_ds : dict[str,Dataset]|Dataset, model, processor : WhisperProcessor, metric : ComputeMetrics, batch_size=3, use_prompt=False, compute_callsign_wer=True, wer_for_AG_existing_only=False, NOP_wer_for_AG_existing_only=False, callsigns_name_in_ds = None, ignore_case=False) -> dict[str,dict]:
    ...
    if (isinstance(test_ds, Dataset) and not use_prompt and not NOP_wer_for_AG_existing_only):
        ...
        return out
    elif (isinstance(test_ds, Dataset) and not use_prompt and NOP_wer_for_AG_existing_only):
        print(f"SINGLE DATASET, NO PROMPT EVAL, ONLY AG EXISTING, batch size {batch_size}")
        # WHEN using PROMPT, so far we can test only one sample at a time with one promp
        # because yet we cannot handle different prompts for different samples in the same batch
        # setup the dataloader
        out = {}
        ...
        if (compute_callsign_wer):
            ...
            avg_callsign_wer = total_callsigns_wer / max(total_callsigns_count,1)
            print(f"allds > wer: {wer} loss: {loss} | callsign wer: {avg_callsign_wer} completely correct: {totaly_callsigns_correct}/{total_callsigns_count}")
            out['allds'] = {'wer':wer,'loss':loss,'callsign_wer':avg_callsign_wer,'callsign_count':total_callsigns_count,'callsign_completely_correct':totaly_callsigns_correct}
        else:
            print(f"allds > wer: {wer} loss: {loss}") # PRINTOUT
            out['allds'] = {'wer':wer,'loss':loss}
...
def parse_args():
    ...
    parser.add_argument('--setup', type=str, required=False, default=None, help='File with params for this script')
    
    parser.add_argument("--metric", type=str, default="wer", help="Metric to evaluate, possible only [wer]")
    parser.add_argument("--datasets", nargs='+', help="List of dataset names to evaluate. Possible only [atco_en_ruzyne, atco_en_stefanik, atco_en_zurich, atco_fr, atco_other_lang, hiwire_fr, hiwire_gr, hiwire_sp, malorca, nato, uwb]")
    parser.add_argument("--datasets_basedir", type=str, default="./data/", help="Base directory for datasets, where the datasets are stored")
    parser.add_argument("--models", nargs='+', help="List of models to evaluate or folder with stored checkpoints.")
    parser.add_argument("--checkpoints_eval", action="store_true", help="This option tells that --models contains folder with checkpoints, that should be evaluated")  # default is False
    parser.add_argument("--same_processor", action="store_true", help="If checkpoints_eval is set. Whether to use same processor for all models")  # default is False
    parser.add_argument("--output_file", type=str, help="File to store the results of evaluation")
    parser.add_argument("--separate_ds", action="store_true", help="Whether to evaluate datasets separately")  # default is False
    parser.add_argument("--overwrite", action="store_true", help="Whether to overwrite the output file")  # default is False
    parser.add_argument("--batch_size", type=int, default=4, help="Batch size for evaluation")  # default is 4
    parser.add_argument("--eval_description", type=str, default="", help="Description of the evaluation")  # default is empty string
    parser.add_argument("--use_prompt", action="store_true", help="Whether to use prompt for evaluation")  # default is False
    parser.add_argument('--ignore_case', action='store_true', help='Ignore case when computing WER')
    parser.add_argument("--self_prompt", action="store_true", help="Whether to use reference transcripts as prompt")  # default is False
    parser.add_argument("--eval_callsigns", action="store_true", help="Whether to evaluate callsigns")  # default is False
    parser.add_argument('--callsigns_name_in_ds', type=str, help='Name of the callsigns in the dataset')
    parser.add_argument("--transcription_name_in_ds", type=str, help='Name of the transcription in the dataset, that will be used for evaluation')
    parser.add_argument("--prompt_name_in_ds", type=str, help='Name of the prompt in the dataset, that will be used for evaluation')
    parser.add_argument('--wer_for_AG_existing_only', action='store_true', help='Compute WER only for utterances with existing callsigns')
    parser.add_argument('--NOP_wer_for_AG_existing_only', action='store_true', help='Compute WER only for utterances with existing callsigns, but without using prompts')
