...
def parse_args():
    ...
    parser.add_argument('--setup', type=str, required=False, default=None, help='Path to the training setup file')
    
    # training_setup
    parser.add_argument("--model_path", type=str, default="openai/whisper-tiny", help="Path to the model, which should be trained")
    parser.add_argument("--continue_from_checkpoint", action='store_true', help="Whether to continue training from checkpoint (model is considered then as checkpoint) or not")
    parser.add_argument("--train_datasets", nargs='+', help="List of training datasets to be used for training. Possible values: apimod, atco_en, atco_fr, hwir, malorca, nato, uwb")
    parser.add_argument("--dropout", type=float, default=None, help="Dropout value to be used in the model. If None, 0.0 value is used.")
    parser.add_argument("--datasets_root_dir", type=str, default="./data", help="Path to the root directory with datasets")
    parser.add_argument("--use_prompt", action='store_true', help="Whether to use prompt for training or not")
    parser.add_argument("--self_prompt", action='store_true', help="Whether to build and use reference transcripts as prompt  for training or not")
    parser.add_argument("--transcription_name_in_ds", type=str, default="full_ts", help="Name of the transcription in the dataset")
    parser.add_argument("--prompt_name_in_ds", type=str, default="prompt_fullts_1G_4B", help="Name of the prompt in the dataset")
    parser.add_argument("--freeze_encoder", action='store_true', help="Whether to freeze encoder or not")
    
    # training_args
    parser.add_argument("--output_dir", type=str, default="", help="Path to the output directory, where trained model and checkpoints will be saved")
    parser.add_argument("--per_device_train_batch_size", type=int, default=4, help="Batch size per device during training, default is 4")
    parser.add_argument("--gradient_accumulation_steps", type=int, default=4, help="Number of gradient accumulation steps, default is 4")
    parser.add_argument("--learning_rate", type=float, default=1e-5, help="Learning rate for the optimizer, default is 1e-5")
    parser.add_argument("--warmup_ratio", type=float, default=0.12, help="Warmup ratio for the learning rate scheduler, default is 0.12")
    parser.add_argument("--weight_decay", type=float, default=0.0 , help="Weight decay for the optimizer, default is 0.0")
    parser.add_argument("--gradient_checkpointing", action='store_true', help="Whether to use gradient checkpointing or not")
    parser.add_argument("--fp16", action='store_true', help="Whether to use mixed precision training (fp16) or not")
    parser.add_argument("--save_strategy", type=str, default="epoch", help="Save strategy for the model checkpoints, default is 'epoch'")
    parser.add_argument("--num_train_epochs", type=int, default=10, help="Number of training epochs, default is 10")
    parser.add_argument("--per_device_eval_batch_size", type=int, default=4, help="UNUSED! Batch size per device during evaluation, default is 4")
    parser.add_argument("--predict_with_generate", action='store_true', help="UNUSED! Whether to use generate method for prediction or not")
    parser.add_argument("--generation_max_length", type=int, default=448, help="Maximum length of the generated sequence, default is 448")
    parser.add_argument("--logging_steps", type=int, default=30, help="Number of steps between logging, default is 30")
    parser.add_argument("--report_to", nargs='+', default=["tensorboard"], help="List of reporting tools to be used, default is ['tensorboard']")
    parser.add_argument("--metric_for_best_model", type=str, default="wer", help="UNUSED! Metric to be used for selecting the best model, default is 'wer'")
    parser.add_argument("--greater_is_better", action='store_true', help="UNUSED! Whether the metric is greater is better or not")
    parser.add_argument("--push_to_hub", action='store_true', help="UNUSED! Whether to push the model to the hub or not")
