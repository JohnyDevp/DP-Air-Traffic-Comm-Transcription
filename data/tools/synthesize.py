...
if __name__ == "__main__":
    ...
    feature_extractor = WhisperFeatureExtractor.from_pretrained("BUT-FIT/whisper-ATC-czech-full")
    tokenizer = WhisperTokenizer.from_pretrained("BUT-FIT/whisper-ATC-czech-full", task="transcribe")
    
    # do resampling by reloading of the data, and change the content of the dataset to FEATURES and LABELS
    dd_dsdict = dd_dsdict.map(prepare_dataset, remove_columns=dd_dsdict.column_names["train"], num_proc=4)
    
    # save the dataset
    dd_dsdict.save_to_disk(os.path.join(RECORDINGS_DIR, "dataset.fortrainig"))